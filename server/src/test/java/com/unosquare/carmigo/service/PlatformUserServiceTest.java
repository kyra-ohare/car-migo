...
public class PlatformUserServiceTest {
  ...
  public void get_PlatformUser_By_Id_Returns_GrabPlatformUserDTO() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      final GrabPlatformUserDTO grabPlatformUserDTO = platformUserService.getPlatformUserById(1,
          any(AppUser.Current.class));
  ...
  public void patch_PlatformUser_Returns_GrabPlatformUserDTO() throws Exception {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      final GrabPlatformUserDTO grabPlatformUserDTO = platformUserService.patchPlatformUser(platformUserFixture.getId(),
          patch, any(AppUser.Current.class));
  ...
  public void delete_PlatformUser_Returns_Void() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      platformUserService.deletePlatformUserById(1, any(AppUser.Current.class));
      verify(platformUserRepositoryMock).findById(anyInt());
    }
  }
  ...
  public void get_Driver_By_Id_Returns_GrabDriverDTO() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      final GrabDriverDTO grabDriverDTO = platformUserService.getDriverById(1, any(AppUser.Current.class));
  ...
  public void create_Driver_Returns_GrabDriverDTO() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      final GrabDriverDTO grabDriverDTO = platformUserService.createDriver(anyInt(), createDriverDTOFixture,
          any(AppUser.Current.class));
  ...
  public void delete_Driver_By_Id_Returns_Void() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      platformUserService.deleteDriverById(1, any(AppUser.Current.class));
      verify(driverRepositoryMock).findById(anyInt());
    }
  }
  ...
  public void get_Passenger_By_Id_Returns_GrabPassengerDTO() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      final GrabPassengerDTO grabPassengerDTO = platformUserService.getPassengerById(1, any(AppUser.Current.class));
  ...
  public void create_Passenger_Returns_GrabPassengerDTO() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      platformUserService.createPassenger(anyInt(), any(AppUser.Current.class));
  ...
  public void delete_Passenger_By_Id_Returns_Void() {
    try (final MockedStatic<AuthenticationUtils> mocked = mockStatic(AuthenticationUtils.class)) {
      ...
      mocked.when(() -> AuthenticationUtils.verifyUserPermission(anyInt(), any(AppUser.Current.class)))
          .thenAnswer(Answers.RETURNS_SELF);
      platformUserService.deletePassengerById(1, any(AppUser.Current.class));
      verify(passengerRepositoryMock).findById(anyInt());
    }
  }
}
