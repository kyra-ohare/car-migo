...
//    @Query(value = "SELECT j FROM Journey j INNER JOIN j.driver d WHERE d.id = ?1")
  ...
  @Query(value = "SELECT j"
          //"SELECT  j.id, j.createdDate, j.dateTime, j.locationFrom, j.locationTo "
      + " FROM Journey j WHERE j.id = ?1")
...
//    SELECT pj.id, j.id, "
//        + "j.createdDate, j.dateTime, j.locationFrom, j.locationTo, j.driver "
//        + "FROM Journey j "
